...
- `CASE-J9W4D` 必须硬拦截：它对目标读者太常识、太容易想到；只引入 `disengagement` 一词却没有展开。“有增量”不等于“有足够增量”。
- `CASE-Q3L7X` 继续硬拦截。
...
- `CASE-M5A8S` 只能判边缘或拦截，绝不能放行：类比本身陌生却没有讲清，读者不知道它究竟说明什么；信息量、新知和解释均不足，问题不只是外部证据缺失。
...
`CASE-K7M2Q` 与 `CASE-V8H3L` 的正例身份不变；`CASE-B9Q5J` 与 `CASE-L3E7W` 的 reference 身份不变，只作参考、不进入硬门。以上反例冻结的是这些固定对象没有达到足够增量、方向、深度、逻辑或可理解性门槛；不是因为其题材、学科、概念、长度或写法被永久禁止。
