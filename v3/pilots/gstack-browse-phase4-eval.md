...
状态：迷你成品页已按用户反馈重写，等待用户复评三问；资产卡冷启动第二轮复测通过。
...
| 非工程师可读性 | 需要用户复评 | 硬标准改为“只认识 cookie 的读者能读完”；是否达标由用户复评三问判定。 |
...
- 迷你成品页仍是切片，不是完整包；没有逐文件解释全部命令和测试。
- 为了让非工程读者能读完，技术解读做了抽象；正式样包需要补 source map 和逐模块追溯。
