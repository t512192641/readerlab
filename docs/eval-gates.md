...
## V2 Reader Product Shape 验收

Reader-facing 产品形态以 `docs/readerlab-v2-reader-product-shape.md` 为准。这里的验收不是安装 smoke，不是 route smoke，也不是 JSON / contract 层结构通过。它只判断普通读者最终打开的内容是否像 ReaderLab 陪读包。

ReaderLab V2 必须把两条线分开验收：

- 图书 / 长文线：读者打开后是在读书或长文正文，带自然方位说明、结构定位和正文旁陪读。
- Skill / 工程材料线：读者打开后是在读一个工程 / Agent workflow 的陪读包，包含净化正文主读页、技术负责人解说页和设计资产卡。

两条线共享正文优先、audit 分离、机器状态不冒充人工验收等底层原则，但不得合并成一套通用 reader gate。
...
```text
[ ] 入口页用自然中文说明材料、覆盖范围和阅读起点。
[ ] 结构地图说明全书/长文结构，当前章节或阅读单元的位置清楚。
[ ] 正文阅读页以原文正文为主体，保持原章节顺序。
[ ] EPUB spine 文件名、抽取占位词、fixture/sample/contract proof 语言不出现在主阅读面。
[ ] audit 路径、source id、hash、machine_status/human_status 不作为主阅读内容出现。
[ ] AI 陪读贴近正文锚点，解决具体阅读问题，而不是通用阅读指令。
[ ] 只覆盖章节时明确章节范围；声明整书时必须有全书闭环。
[ ] 独立 reader evaluation 记录读者体验结论，不能用 route smoke 替代。
```

失败条件：

```text
[FAIL] 读者页标题仍是“局部样本”“contract proof”“fixture”等工程语言。
[FAIL] 正文前先出现路径、来源文件、机器状态或审计说明。
[FAIL] 章节正文被 AI 摘要、导读或高阶讲解替代。
[FAIL] 抽取噪音如“未知”、spine 文件名、HTML 文件名进入阅读正文。
[FAIL] 章节样本被说成整本书验收通过。
```
...
```text
[ ] 入口页说明这是哪个 Skill 包/工程材料，解决哪类问题，从哪里开始读。
[ ] 多 Skill 包按问题域或能力域组织，不按文件列表或目录顺序平铺。
[ ] 每个已完成核心 Skill 或模块都有净化正文主读页。
[ ] 净化正文保留用途、触发条件、用户意图、流程、约束、失败条件、输出要求和设计亮点。
[ ] 安装命令、重复模板、执行外壳、机器状态、路径、hash 等噪音不污染主读页。
[ ] 技术负责人解说页面向产品负责人解释为什么这样设计、为什么有效、不这样做会坏在哪里、代价和边界是什么。
[ ] 设计资产卡能让无背景 Agent 不打开原始 source 也能理解和复用。
[ ] 被剥离的壳子若有工程设计含义，进入 audit、附录、技术解说或资产卡，而不是丢失。
[ ] 验收分别记录普通读者体验、产品负责人预期和无背景 Agent 复用，不合并成一句“读者视角 OK”。
```

失败条件：

```text
[FAIL] 输出只有目录、manifest、状态表、分组导读或“待生成”目标路径。
[FAIL] 已完成 Skill 没有净化正文主读页。
[FAIL] AI 能力介绍、摘要或术语解释替代净化正文。
[FAIL] 技术解说页只是实现日志、字段解释或净化正文换皮复述。
[FAIL] 资产卡离开原始 source 就看不懂、不能调用或不知道边界。
[FAIL] 针对 GSTACK 写专门规则，而不是抽成通用 Skill / 工程材料规则。
```
