...
[ ] 书籍/长文过 Body Track Gate：完整正文存在，或显式链接到 `10_一手正文/` 下的完整正文文件
[ ] 高阶讲解通过和阅读包通过状态分开
...
[ ] 书籍/长文页面若无完整正文或 `10_一手正文/` 显式链接，只能标为 `high_order_explanation_pass`，不能标为 `reader_package_pass`
[ ] Skill/工程材料有净化正文
...
[FAIL] 主阅读页变成普通摘要。
[FAIL] AI 重点替代了一手正文。
[FAIL] 书籍/长文页面没有完整一手正文轨或 `10_一手正文/` 显式链接，却标为 reader_package_pass。
