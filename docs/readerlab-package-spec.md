...
ReaderLab 包必须先满足正文主体，再在同一阅读页里提供就近 AI 陪读和 audit。不同材料类型的主体不同：

- 书籍 / 长文：`10_中文精读/` 下每个阅读单元是一个 Markdown 文件，文件内先保留原样正文和章节顺序，再在正文附近插入自然陪读块。默认不压缩、不改写、不重述正文；除非用户明确要求整理原文，否则只做空格、空行、断行等轻量排版清理。AI 讲解不能替代正文，也不能被拆到远离正文的独立文件里。
- 书籍 / 长文的状态前置门：只有阅读页包含完整章节正文，或显式链接到同页/同单元的完整正文，才可以进入 `reader_package_pass` 判断。只有高阶讲解、摘要、导读、锚点清单或正文贴合证明时，最多只能标为 `high_order_explanation_pass`。
- Skill / 工程材料：阅读页内放净化正文和就近陪读。净化正文剥离安装命令、重复模板、执行外壳、机器状态、路径、hash、调试信息，但保留用途、触发条件、用户意图、核心流程、约束、失败条件、输出要求和设计亮点。
...
`10_中文精读/` 是读者第一次阅读和批注的核心。每个阅读单元必须是单个可批注 Markdown 页，正文、陪读和必要提醒就近出现。
...
> 陪读：必要时给出边界、误读防护或迁移提示
```

规则：

- 正文 / 净化正文是页面主体，必须先出现并占主要篇幅。
...
- AI 讲解不能替代正文。
- 设计资产提炼只在 Skill/工程/Agent 工作流材料中出现，并应靠近对应正文或进入 audit/附录。
- 没有真实边界或误读风险时，不强行凑内容。
- 降级/拒绝理由默认进入 `audit/rejected-downgraded.md`，读者页只保留少量自然语言误读提醒。
- 完整原文、hash、命令行、机器执行视图默认进入 `audit/`。
- 不默认生成“批注问题文件”。真实批注由用户在 Obsidian 阅读页正文附近添加；ReaderLab 只需保证正文和陪读能被就地批注。

## AI 陪读

`20_全书导读/` 只放全书级或包级 orientation。章节级陪读必须贴在 `10_中文精读/` 对应阅读页内，不能让读者在 A 文件看正文、B 文件看陪读。
...
批注与讨论不替代正文附近批注，只在确有需要时提供索引和汇总：

- `comments-index.md`：批注索引。
- `codex-replies.md`：必要时汇总 Codex 回复。

真实批注必须贴近正文或保留插件原始存储格式。默认不生成预设批注问题页。
