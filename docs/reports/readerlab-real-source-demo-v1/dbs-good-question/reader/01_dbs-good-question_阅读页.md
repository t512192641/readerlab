...
- `Agent`：这里指能根据输入材料执行推理、生成、检查或行动建议的 AI 工作单元。
- `Skill`：这里指一组可复用的任务说明、触发条件、流程、约束和输出格式。
- `hard to vary`：指解释或方案足够具体，不能随便替换几个词还同样成立。
- `/dbs`：原工具体系里的导航入口；作为阅读材料时可理解为“下一步路由说明”。

## 高阶讲解

这份 Skill 的主体不是在教用户“怎么把问题说得漂亮”，而是在把一个模糊请求改造成 Agent 能处理的推理对象。它反复强调的第一件事是钉住现象：如果问题只是“为什么做不好”“能不能自动化”，Agent 会被迫用默认假设填空；如果问题被压成一个可观察的断点，比如收藏高但咨询少、咨询多但付款少、想自动化但规则不清，后续解释才有边界。

它的第二层价值是把“好问题”定义成一组约束，而不是一句聪明的提问。对象、目标、冲突、约束、反馈这五项检查，实际上是在压缩 Agent 的搜索空间：对象让它知道分析谁，目标让它知道要解释还是决策，冲突让它找到异常，约束让它不乱改现实条件，反馈让它知道什么证据能推翻或修正解释。这个结构使候选解释可以被批评，而不是停在听起来合理的泛泛判断。

第三层是自动化边界。它没有把“Agent 能不能做”处理成 yes/no，而是拆成 A/B/C/D 四档：高度自动化、半自动化、辅助推理、暂不适合自动化。这个分档对产品负责人很关键，因为很多流程失败不是因为 Agent 不会写文本，而是因为边界、变量、反馈或行动权限没有进入系统。Skill 要求先区分 Agent 可以生成解释、可以生成好解释、还是可以真正闭环解决问题，这能避免把现实反馈缺失的问题包装成自动化能力。

最后，输出格式的设计体现了一个交互判断：默认不要把评分表甩给用户，而是先给断点、清晰度、最大缺口和最小下一步。只有用户要求严格审计时才展开打分表。这让 Skill 更像一个问题教练，而不是一个格式审查器；它把严谨性留在背后，把用户当前最需要的抓手放在前面。

## 设计资产卡

### 卡 1：现象优先的问题澄清

- 适用场景：用户带着大而散的问题来找 Agent，例如转化差、产品没人买、流程能否自动化。
- 解决问题：防止 Agent 直接回答抽象问题，先把任务压到可观察断点上。
- 原材料依据：原文“原则 1：好问题先钉现象”以及坏问题 / 好问题对照。
- 可复用做法：先要求用户给出现象、对象、时间范围、数据或场景，再进入解释和方案。
- 使用前提：至少能获得一个真实现象；如果只有情绪或愿望，需要先追问事实。
- 失败风险：如果过早要求数据，可能让低成熟度用户卡住；需要保留低置信候选解释作为抓手。
- 什么时候不要用：用户已经给出完整材料并明确要求执行时，不应再机械地退回“请先描述现象”。

### 卡 2：五项约束检查

- 适用场景：需要把自然语言问题转成可交给 Agent、团队或工具执行的问题说明书。
- 解决问题：让问题具备对象、目标、冲突、约束、反馈，减少 Agent 脱靶和脑补。
- 原材料依据：Phase 2 的五项检查表和 0-2 分评分规则。
- 可复用做法：把任何复杂请求先过一遍五项检查；低分项不直接补脑，而是转成最小追问。
- 使用前提：用户愿意接受一次澄清；任务不是必须立即执行的纯操作型请求。
- 失败风险：评分机制如果暴露太早，会让用户感觉被审判，所以原文要求默认不展示完整评分表。
- 什么时候不要用：快速创意、开放探索或纯情绪陪伴场景中，五项审计可能显得过重。

### 卡 3：自动化程度分档

- 适用场景：判断一个业务流程、内容流程或个人工作流能否交给 Agent 自动处理。
- 解决问题：把“能不能自动化”拆成边界、变量、反馈、检验、行动、规律稳定性，避免泛泛承诺。
- 原材料依据：Phase 3 的六个判断项和 A/B/C/D 四档输出。
- 可复用做法：先标出可自动化部分、人类必须判断的部分、需要反馈回流的部分，再决定是否开发自动化。
- 使用前提：任务边界和反馈入口至少能被描述；否则只能判 C 或 D。
- 失败风险：团队可能把“可辅助推理”误读成“可自动解决”，导致过度自动化。
- 什么时候不要用：一次性、高度人际博弈、反馈不可获得的任务，不应为了形式完整强行分档。

### 卡 4：抓手先于审计

- 适用场景：用户问题模糊但期待立刻看到推进方向。
- 解决问题：避免一上来给审计表造成挫败，同时不牺牲严谨边界。
- 原材料依据：原则 6 和输出格式 A：先写断点、当前清晰度、最大缺口，再补半成品问题说明书。
- 可复用做法：先用 1-2 句话指出最可能的断点，再声明置信度和缺口，最后只给一个最小下一步。
- 使用前提：Agent 能识别至少一个合理断点；否则应坦白信息不足。
- 失败风险：低置信候选解释如果标注不清，会被用户当成确定诊断。
- 什么时候不要用：合规审计、事故复盘、财务判断等高风险场景，应优先展开证据和限制条件。
