# Expert Teaching Skill v0.1.1 Code Review Brief
...
8. 0.1.0／0.1.1 是否可明确共存、错误版本是否 fail closed？
9. 来源 allowlist 的三种状态、redirect、输出 URL 边界和 replay hash 是否准确？
...
- Skill entry and resources: `.agents/skills/readerlab-book-expert-teaching/`，重点为独立的
  `versions/0.1.1/`、`CURRENT_VERSION` 与 `scripts/run-current.py`；根入口是不可变 0.1.0 历史实现。
- Deterministic tests: `tests/test_skill.py`、`tests/fixtures/t2.38-replay.json` 和
  `tests/fixtures/t2.38-source-allowlist.json`。
...
模板或脚本的未来修改都需要新 Skill 版本和 change receipt；0.1.0 尚未运行迁移样本，0.1.0 与 0.1.1 结果
