...
`PRODUCT-DECISIONS.md` 的“透镜系统的原始产品目标”与“模型分层与日常生产目标”拥有本阶段产品事实；本路线不复制这些决议，只把它们编译成当前执行顺序。

当前执行顺序是：产品接受的正例目标包；目标档模型在相同材料上的独立复现与分层诊断；未见章节验证；建立并验证最小生产合同与安全 harness；完成后才根据真实生产失败决定是否以及如何启动裁判。前三轮均未形成产品接受的正例：第一轮上下文截断并由预分配框架驱动炫技式扩写；第二轮过度纠偏成局部短注释；第三轮虽恢复深度，却由中央先定问题、专家围绕指定问题找锚点，第一条与产品负责人既有认识高度重合且挑错章节重点，第二条只有部分新机制但被绕行和复写淹没。下一轮仍停留在正例目标包阶段，改为稳定背景专家独立顺序阅读，并对同一专家初稿比较专家自改与独立 Writer 编辑，分别诊断知识质量和组织质量。`blueprints/PIPELINE-MAP.md` 只拥有当前可替换的路线假设。
