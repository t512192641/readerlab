...
Version selection is explicit. A run created by `0.1.0` must be verified through the immutable `0.1.0` entry; a `0.1.1` entry refuses it. The current dispatcher never falls back to another version when a run's recorded version differs.
