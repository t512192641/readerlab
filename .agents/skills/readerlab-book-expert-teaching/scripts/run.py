#!/usr/bin/env python3
"""Unified deterministic entry point for ReaderLab Book Expert Teaching v0.1."""
...
def _allowed_sources_text(urls: list[str]) -> str:
    if not urls:
        return "- (source map contains no URL; remain closed to search)"
    return "\n".join(f"- `{url}`" for url in urls)


def _read_set(run: Path, role: str, local_files: list[str], urls: list[str]) -> dict[str, Any]:
    return {
        "schema": "readerlab-book-expert-teaching/read-set/v1",
        "role": role,
        "exact_local_files": local_files,
        "allowed_source_urls": urls,
        "network": "allowlisted source URLs only; no open-ended search",
        "forbidden": [
            "old runs",
...
def _write_task_files(run: Path, urls: list[str]) -> None:
    display = run.name
    values = {
        "RUN_NAME": display,
        "RUN_DIR": os.fspath(run),
        "ALLOWED_SOURCES": _allowed_sources_text(urls),
    ...
    product_task = (
        f"# Product pack task · {display}\n\n"
        "Build only after `SOURCE_FIDELITY_PASS + TEACHING_PASS`. The controller must render the fixed source "
        "into readable Markdown, append the complete Expert course and fixed product questions, and reject raw "
        "XHTML, source maps, technical scores, Writer/route material, old drafts, and M1/M2/M3 labels.\n"
    )
...
def _initial_state(run: Path, original: dict[str, Path], copied: dict[str, dict[str, Any]]) -> dict[str, Any]:
    created = now()
    return {
        "schema": SCHEMA,
        "skill_version": SKILL_VERSION,
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        urls = urls_from_source_map(run / "inputs/source-map.md")
        _write_task_files(run, urls)
        _write_read_set(run, "control/expert-read-set.json", _read_set(run, "Expert teaching", EXPERT_LOCAL_READ_SET, urls))
        _write_read_set(run, "control/review-read-set.json", _read_set(run, "Independent review", REVIEW_LOCAL_READ_SET, urls))
        ...
        state = _initial_state(run, original, copied)
...
def cmd_seal_expert(args: argparse.Namespace) -> None:
    ...
    if re.search(r"\bM[123]\b", draft_text):
        raise SkillError("Expert teaching draft must not self-label depth with M1/M2/M3")
    stage_freeze(run, "stage-1.sha256", ["raw/expert-teaching-draft.md", "raw/expert-teaching-source-map.md"])
...
def cmd_seal_review(args: argparse.Namespace) -> None:
    ...
    stage_freeze(run, "stage-2.sha256", ["acceptance/expert-teaching-review.md"])
    state = transition_with_fields(
        run,
        "REVIEW_OPEN",
        "REVIEW_TERMINAL",
...
def _verify_read_sets(run: Path) -> None:
    ...
    for value in (expert, review):
        if value.get("network") != "allowlisted source URLs only; no open-ended search":
...
def cmd_verify(args: argparse.Namespace) -> None:
    ...
    if state["status"] in {"REVIEW_OPEN", "REVIEW_TERMINAL", "PRODUCT_READY"}:
        ...
        for key, path in (("draft", draft), ("source_map", source_map)):
...
def _argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ReaderLab Book Expert Teaching Skill v0.1")
