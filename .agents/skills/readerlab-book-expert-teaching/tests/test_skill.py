...
RUN_SCRIPT = SKILL_DIR / "versions/0.1.1/scripts/run.py"
CURRENT_SCRIPT = SKILL_DIR / "scripts/run-current.py"
...
class ExpertTeachingSkillTests(unittest.TestCase):
    ...
    def init_run(self, script: Path = RUN_SCRIPT, version: str = "0.1.1") -> Path:
    ...
    def write_expert(self, run: Path, body: str = "# 教学\n完整课程。\n", source_map: str | None = None) -> None:
    ...
    def write_review(self, run: Path, fidelity: str = "SOURCE_FIDELITY_PASS", teaching: str = "TEACHING_PASS", body: str = "Evidence.\n") -> None:
    ...
    def test_reference_only_url_can_be_cited_but_blocked_url_cannot(self) -> None:
        ...
        (run / "acceptance/expert-teaching-review.md").write_text(
            ...
            "Reference: https://blocked.example.invalid/source\n",
            encoding="utf-8",
        )
    ...
    def test_current_dispatcher_targets_011(self) -> None:
        run = self.init_run(script=CURRENT_SCRIPT)
        state = json.loads((run / "run.json").read_text(encoding="utf-8"))
        self.assertEqual(state["skill_version"], "0.1.1")

    def _assert_fixture_hashes(self, fixture: dict[str, object]) -> None:
        for key in ("source", "source_map", "source_allowlist", "expert_draft", "expert_source_map", "review"):
            record = fixture[key]
    ...
    def test_t238_fixture_replay_checks_all_hashes_product_identity_and_archive(self) -> None:
        ...
        self.assertEqual(fixture["replay_skill_version"], "0.1.1")
        ...
        result = invoke(
            "init", "--source", source, "--framework", framework, "--source-map", source_map,
            "--source-allowlist", source_allowlist, "--run", run, "--skill-version", "0.1.1",
        )
        self.assertEqual(result.returncode, 0, result.stderr)
        ...
        self.assertEqual(package_sha, fixture["expected_product"]["v0.1.1_sha256"])
        self.assertEqual(invoke("verify", "--run", run).returncode, 0)
        ...
        for key in ("source", "source_map", "expert_draft", "expert_source_map", "review"):
            record = fixture[key]
            frozen_path = run / {
                "source": "inputs/source.md",
                "source_map": "inputs/source-map.md",
